"""
=============================================================================
Pokemon Card Value Analytic Tool — Model 1: Card Identifier Engine (varian CLIP)
=============================================================================
INI FILE EKSPERIMENTAL, TERPISAH dari card_identifier.py yang sudah stabil
pakai MobileNetV3. Satu-satunya perbedaan dari card_identifier.py: backbone
ekstraksi fitur diganti dari MobileNetV3 (classifier ImageNet generic) ke
CLIP ViT-B-32 (dilatih kontrastif image-text, jauh lebih tajam untuk tugas
image retrieval/pencocokan visual).

SEMUA logika lain (alignment berlapis, kalibrasi confidence berbasis margin,
ORB re-rank, TTA, dsb) PERSIS SAMA dengan card_identifier.py -- supaya kita
bisa isolasi dengan jelas: peningkatan/penurunan hasil yang terjadi murni
karena backbone-nya, bukan karena hal lain ikut berubah.

WAJIB jalankan `build_card_index_clip.py` dulu sebelum pakai file ini --
index & mapping-nya beda dari yang dipakai card_identifier.py (dimensi
embedding CLIP beda dari MobileNet, tidak saling kompatibel):
...
INDEX_PATH = os.path.join(MODEL_DIR, "card_embeddings_clip.index")
MAP_PATH = os.path.join(MODEL_DIR, "card_id_map_clip.json")
...
# Pilihan model CLIP -- HARUS SAMA PERSIS dengan yang dipakai di build_card_index_clip.py,
# kalau beda maka embedding query & index tidak "berbicara bahasa yang sama".
...
class CardIdentifier:
    ...
    def _load_resources(self):
        if not os.path.exists(self.index_path) or not os.path.exists(self.map_path):
            raise FileNotFoundError(
                "Berkas index CLIP belum dibuat! Silakan jalankan "
                "`python backend/models/build_card_index_clip.py` terlebih dahulu."
            )
        ...
        self.transform = preprocess  # preprocess dari open_clip sudah termasuk resize+normalize yg sesuai
    ...
    def _extract_embedding(self, pil_img):
        ...
        with torch.no_grad():
            feat = self.clip_model.encode_image(tensor_img).cpu().numpy().astype("float32")
        return feat
...
if __name__ == "__main__":
    print("Testing CardIdentifier (CLIP variant) engine initialization...")
    identifier = CardIdentifier()
    print("CardIdentifier varian CLIP siap digunakan!")