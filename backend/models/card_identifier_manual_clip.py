"""
=============================================================================
Pokemon Card Value Analytic Tool — Manual Photo Scanner (varian CLIP)
=============================================================================
Salinan card_identifier_manual.py yang memakai backend CLIP
(card_identifier_clip.py) alih-alih MobileNet -- supaya bisa dites
berdampingan tanpa bongkar-pasang import. Semua fitur (jendela pilih file,
--expected_card_id, full-rank search) identik dengan versi MobileNet.
...
Cara pakai (sama seperti versi MobileNet, cuma nama file beda):
    python card_identifier_manual_clip.py

Jendela File Explorer akan otomatis terbuka untuk memilih foto. Setelah satu
kartu selesai diproses, jendela akan terbuka lagi untuk scan berikutnya --
tinggal klik Cancel kapan saja untuk berhenti. Kamu juga akan ditanya
(opsional, boleh Enter untuk lewati) card_id yang benar setiap habis scan,
supaya bisa langsung divalidasi.

Cara pakai alternatif (path file & card_id langsung lewat command line, buat
automasi/scripting):
    python card_identifier_manual.py foto_qwilfish.jpg --expected_card_id me4-52

CATATAN: ORB re-rank AKTIF SECARA DEFAULT di script ini (terbukti dari
pengujian nyata cukup membantu, dan latency ~0.5-1 detik per scan tidak
masalah untuk pemakaian manual begini). Tambahkan --no_orb_rerank kalau mau
tes cepat tanpa verifikasi ORB.

Ini penting untuk membedakan 2 kemungkinan akar masalah:
  - Kalau kartu yang benar ranking-nya DEKAT top (mis. #3-#20) tapi kalah
    tipis similarity-nya -> ini soal ranking/diskriminasi pada kandidat mirip,
    bisa dibantu re-ranking (ORB) atau kalibrasi ulang.
  - Kalau kartu yang benar ranking-nya JAUH (mis. #5000 dari ~20rb) dan
    similarity-nya jauh di bawah top-1 -> fitur/embedding memang tidak cukup
    diskriminatif untuk kartu ini. Bukan lagi soal kalibrasi, tapi soal
    backbone/feature extraction (lihat CHANGELOG untuk opsi upgrade).

PENTING: gunakan foto ASLI (belum pernah di-crop/align sebelumnya), bukan file
`debug_*_aligned.jpg` yang sudah dipotong dari sesi sebelumnya -- kalau file
yang sudah rapat ke tepi kartu dimasukkan lagi, alignment akan mencoba
mendeteksi ulang kontur pada gambar yang sudah "full-frame" dan bisa menolak
proses align (ini memang perilaku aman by design, bukan bug, tapi bisa
membingungkan kalau tidak disadari).
=============================================================================
"""
...
def pick_file_dialog():
    ...
    mirip pengalaman upload gambar di chat AI. Mengembalikan string kosong
    kalau user menekan Cancel/menutup jendela."""
    try:
...
def find_full_rank_from_aligned(identifier, aligned_image_path, expected_card_id, use_tta=False):
    """
    Ekstrak embedding LANGSUNG dari gambar yang SUDAH di-align (tanpa memanggil
    align_card_image lagi -- gambar ini sudah rapat ke tepi kartu), lalu cari
    similarity ke SELURUH kartu di index (bukan cuma top-k), dan laporkan di
    posisi/rank berapa `expected_card_id` sebenarnya berada.
    """
...
def process_one(identifier, image_path, top_k, expected_card_id, use_tta, full_rank_search):
    """Proses satu file foto: identifikasi, cetak hasil, dan validasi opsional."""
    if not os.path.exists(image_path):
        print(f"[error] File tidak ditemukan: {image_path}")
        return

    debug_path = "debug_manual_aligned_clip.jpg"
